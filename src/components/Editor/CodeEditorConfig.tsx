...
const CodeEditorConfig: React.FC<IProps> = props => {
  ...
  return (
    ...
    >
      ...
      <Form.Item name={'mouseWheelZoom'} label="鼠标滚轮缩放">
        <Switch defaultChecked={monacoConfig.options.mouseWheelZoom}></Switch>
      </Form.Item>
      <Form.Item name={'smoothScrolling'} label="平滑滚动">
        <Switch defaultChecked={monacoConfig.options.smoothScrolling}></Switch>
      </Form.Item>
      <Form.Item
      ...
      <Form.Item name={'showUnused'} label="淡出未使用变量">
        <Switch defaultChecked={monacoConfig.options.showUnused}></Switch>
      </Form.Item>
      <Form.Item name={'cursorSmoothCaretAnimation'} label="光标平滑动画">
        <Select
