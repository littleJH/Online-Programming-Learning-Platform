...
import { createProblemLabelApi, getProblemLabelsApi, getProblemListApi, uploadProblemByFileApi, uploadVjudgeProblemApi } from "@/api/problem";
...
export const UploadProblemModal: React.FC<IProps> = (props) => {
  ...
  const handleUpload =
    async (file: RcFile) => {
      ...
      const { data: { code, data: resData } } = await uploadVjudgeProblemApi(JSON.stringify(data))
      console.log('code data ==> ', code, resData)
      if (code === 200) {
        createTagAuto(resData.problem)
      }
      setFailUploadFileList(value => [...value, {
        uid: file.uid,
        name: file.name,
  ...
  const createTagAuto = useCallback(async (problem: IProblem) => {
    ...
    const { code, data: { tagCount } } = res.data
    if (code === 200) {
      let index1 = 0
