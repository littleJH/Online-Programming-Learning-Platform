    ...
    <div className="w-full h-full flex ">
      ...
      <div className="h-12 pr-4 flex items-center">
        {currentState.state === 'info' && currentState.value === '' && (
          <MySvgIcon href="#icon-info" size={iconSize} color={token.colorInfo}></MySvgIcon>
        )}
        {currentState.state === 'info' && currentState.value !== '' && <Loading></Loading>}
        {currentState.state === 'error' && (
          <MySvgIcon href="#icon-error" size={iconSize} color={token.colorError}></MySvgIcon>
        )}
        {currentState.state === 'success' && (
          <MySvgIcon href="#icon-success" size={iconSize} color={token.colorSuccess}></MySvgIcon>
        )}
      </div>
        ...
        {currentState.state === 'success' && runResult && (
          <div className="">
            ...
            <div>
              ...
              <p>
                <span>当前输出：</span>
                <span className={`rounded ${runResult.output === caseSample.output ? 'bg-green-300' : 'bg-red-300'}`}>
                  {runResult.output}
                </span>
              </p>
            </div>
