...
import { Switch, Button, Form, Input, Row, Col, InputNumber, DatePicker } from 'antd'
...
const Competition: React.FC<Iprops> = (props) => {
  ...
  const timeRange: any = [dayjs(competition?.start_time), dayjs(competition?.end_time)]

  useEffect(() => {
    form.setFieldsValue(competition)
    form.setFieldValue('timeRange', timeRange)
    const duration = dayjs(competition?.hack_time).unix() - dayjs(competition?.end_time).unix()
  ...
  const onFinish = () => {
    ...
    Object.keys(formObj).forEach((key) => {
      if (key === 'timeRange') {
        ...
        data.hack_time = dayjs(data.end_time).add(formObj[key], 'minute').format('YYYY-MM-DD HH:mm:ss')
      } else {
    ...
    delete data.user_id
    console.log(data)
    updateCompetitionApi(competition?.id as string, JSON.stringify(data)).then((res) => {
      console.log(res)
  ...
  return (
    <div className="">
      <Form size="large" scrollToFirstError name="problemForm" form={form} onFinish={onFinish} layout="vertical">
